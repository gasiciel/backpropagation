...
def soft_max(wektor):
    exp_wektor = np.exp(wektor)
    suma_wektor_exp = np.sum(exp_wektor)
    return exp_wektor/suma_wektor_exp
...
class Warstwa:
    ...
    def propagacja_w_przod_soft_max(self, macierz):
        self.X = np.insert(macierz, 0, 1, axis=0)
        self.net = self.wagi @ self.X
        self.a = soft_max(self.net)
        return self.a
    ...
    def propagacja_w_tyl_soft_max(self, y, wspolczynnik_uczenia):
        delta_ostatnia = np.sum(y) * self.a - y
        delta_LW = delta_ostatnia @ self.X.T    
        delta_poprzednia = self.wagi.T @ delta_ostatnia
        self.wagi -= wspolczynnik_uczenia * delta_LW
        return delta_poprzednia[1:]
...
class Siec:
    ...
    def fit(self, training_data, epoki=5, wspolczynnik_uczenia=0.01):
        for epoka in range(epoki):
            for X, y in training_data:
                for warstwa in self.warstwy:
                    if warstwa == self.warstwy[-1]:
                        X = warstwa.propagacja_w_przod_soft_max(X)
                    else:
                        X = warstwa.propagacja_w_przod(X)
                
                for warstwa in reversed(self.warstwy):
                    if warstwa == self.warstwy[-1]:
                        blad = warstwa.propagacja_w_tyl_soft_max(y, wspolczynnik_uczenia)
                    else:
...
if __name__ == "__main__":
    ...
    print(odpowiedzi)
    print(f"Skuteczność predykcji {skutecznosc} % - soft-max z cross-entropy")